...
void updateVertexArray()
{
  ...
  // allocate index array, free previous elements if they exist
  if (indices != NULL)
    {
      for (i = 0; i <= qPrev; i++) free(indices[i]);
      qPrev = q;
    }
  free(indices);
  if ((indices = (unsigned int**) malloc((q + 1) * sizeof(unsigned int**))) == NULL) exit(1);

  // allocate count array
  free(count);
  if ((count = (int*) malloc((q + 1) * sizeof(unsigned int*))) == NULL) exit(1);
  ...
  for (i = 0; i <= q; i++) // all vertices will be the same when i=q, but the code is simpler this way
    {
      // allocate indices[i]
      if ((indices[i] = (unsigned int*) malloc((p + 1) * 2 * sizeof(unsigned int))) == NULL) exit(1);
	...
	{	  
	  ...
	  indices[i][2 * j] = i * p + j;
	  indices[i][2 * j + 1] = (i + 1) * p + j;
	}
      // repeat the first two vertices
      indices[i][2 * p] = i * p;
      indices[i][2 * p + 1] = (i + 1) * p;
      count[i] = 2 * (p + 1); // all elements will be equal. last element is not used but this is faster than using modulo
    }
}
...
void drawScene(void)
{
  ...
  glMultiDrawElements(GL_TRIANGLE_STRIP, count, GL_UNSIGNED_INT, (void**)indices, q);
